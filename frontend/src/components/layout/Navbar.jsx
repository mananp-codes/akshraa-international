...
const Navbar = () => {
  ...
  return (
    <nav className="bg-primary-600 text-white shadow-lg sticky top-0 z-50 border-b border-white/5">
      <div className="container-custom">
        <div className="flex items-center justify-between h-[68px]">

          {/* ── Logo ───────────────────────────────── */}
          <Link to="/" className="flex items-center gap-3 group">
            <div className="w-10 h-10 rounded-full flex items-center justify-center border-2 border-gold-500 group-hover:border-gold-300 transition-colors">
              <span className="font-heading font-bold text-lg text-gold-400 group-hover:text-gold-300 transition-colors">A</span>
            </div>
            <div className="hidden sm:block">
              <div className="font-heading font-bold text-lg leading-tight tracking-tight">Akshraa International</div>
              <div className="text-gold-400/90 text-[11px] uppercase tracking-widest2">Quality Textiles Worldwide</div>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-3">
            ...
            {user ? (
              <div className="relative">
                ...
                {/* Dropdown */}
                {userMenuOpen && (
                  <div className="absolute right-0 mt-2 w-52 bg-white text-ink rounded-xl shadow-2xl border border-paper-200 overflow-hidden z-50 animate-slide-up">
                    ...
                    <div className="py-1">
                      ...
                      {user.role === 'seller' && (
                        <Link
                          ...
                          className="flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-gray-50 transition-colors"
                        >
                          <FiUser className="text-primary-600" /> Seller Dashboard
                        </Link>
                      )}
                      {user.role === 'admin' && (
                        <Link
